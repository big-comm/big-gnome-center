  ...
  class TaskbarAppIcon extends AppDisplay.AppIcon {
      ...
      if (!this._isGroupApps) {
        ...
        this._focusedDots.connect('repaint', () => {
          if (!this._dashItemContainer.animatingOut) {
            ...
            let isFocused = communityStyle
              ? this._communityIndicatorFocused
              : true
            ...
            this._drawRunningIndicator(
              this._focusedDots,
              SETTINGS.get_string(
                isFocused ? 'dot-style-focused' : 'dot-style-unfocused',
              ),
              isFocused,
            )
          }
    ...
    _displayProperIndicator() {
      ...
      if (!this._isGroupApps) {
        ...
        if (this._getCommunityIndicatorStyle()) {
          this._communityIndicatorFocused = isFocused
          Utils.stopAnimations(this._focusedDots)
          Utils.stopAnimations(this._unfocusedDots)
          this._unfocusedDots.set_size(0, 0)
          this._unfocusedDots.opacity = 0
          this._focusedDots.set_size(-1, -1)
          this._focusedDots.x_expand = true
          this._focusedDots.y_expand = true
          this._focusedDots.opacity = this._nWindows > 0 ? 255 : 0
          this._focusedDots.queue_repaint()
          return
        }
    ...
    _getCommunityIndicatorStyle() {
      ...
      if (
        focusedStyle == DOT_STYLE.SEGMENTED &&
        unfocusedStyle == DOT_STYLE.SEGMENTED
      )
        return 'hybrid'
      if (
        focusedStyle == DOT_STYLE.METRO &&
        unfocusedStyle == DOT_STYLE.DASHES
      )
        return 'desk-ux'
      return null
    }
    ...
    _getRunningIndicatorColor(isFocused) {
      ...
      if (!isFocused && this._getCommunityIndicatorStyle()) {
        return new Utils.ColorUtils.Color({
          red: 160,
          green: 160,
          blue: 168,
          alpha: 184,
    ...
    _drawRunningIndicator(area, type, isFocused) {
      ...
      if (communityStyle) {
        let scaleFactor = Utils.getScaleFactor()
        let targetLength =
          (communityStyle == 'hybrid' ? 18 : isFocused ? 18 : 8) * scaleFactor
        let length = Math.min(areaSize, targetLength)
        let offset = Math.floor((areaSize - length) / 2)
        let visualScale = length > 0 ? targetLength / length : 1

        area.set_scale.apply(
          area,
          isHorizontalDots ? [visualScale, 1] : [1, visualScale],
        )

        cr.translate.apply(
          cr,
          isHorizontalDots ? [offset, startY] : [startX, offset],
        )
        cr.setSourceColor(bodyColor)
        cr.newSubPath()
        cr.rectangle.apply(
          cr,
          isHorizontalDots ? [0, 0, length, size] : [0, 0, size, length],
        )
        cr.fill()
        cr.$dispose()
        return

...
const ApplicationsMenu = class extends PopupMenu.PopupMenu {
    ...
    _onCapturedEvent(actor, event) {
        const maybeHandleEvent = Main.keyboard?.maybeHandleEvent;
        if (typeof maybeHandleEvent === 'function' &&
            maybeHandleEvent.call(Main.keyboard, event))
    ...
    _syncPositionSource() {
        const centerDeskUxMenu =
            SETTINGS.get_enum('layout') === Constants.LAYOUTS.APP_GRID &&
            SETTINGS.get_string('desktop-layout')
                .trim()
                .toLowerCase()
                .replaceAll(' ', '-')
                .replaceAll('_', '-') === 'desk-ux';
        if (!centerDeskUxMenu) {
    ...
    open(animate) {
        ...
        if (this.actor.get_parent?.() === Main.uiGroup &&
            keyboardBox?.get_parent?.() === Main.uiGroup)
            Main.uiGroup.set_child_below_sibling(this.actor, keyboardBox);
        if (!this._layout)
            this._loadLayout();
        this._layout.reset();

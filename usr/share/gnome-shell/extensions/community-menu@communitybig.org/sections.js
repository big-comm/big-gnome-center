...
export const CategoriesListSection = GObject.registerClass({
    ...
    _init(appsBackend, params = {}) {
        ...
        this._openCascadeOnRight = params.openCascadeOnRight ?? false;
        this._inheritLightStyle = params.inheritLightStyle ?? false;
    ...
    _ensureCategoryMenu(categoryItem, categoryMenuId) {
        ...
        if (!menu) {
            menu = new CategoryAppsMenu(
                ...
                categoryMenuId,
                this._monitorIndex,
                this._openCascadeOnRight,
                this._inheritLightStyle,
                () => this.emit('activated'));
...
export const HybridCategoriesSection = GObject.registerClass({
    ...
    _addCategoryButton(category) {
        let button = this._categoryButtons.get(category);
        if (!button) {
            button = new MiscMenuItems.CategoryMenuItem(
                category,
                this._iconSize,
                false);
            this._categoryButtons.set(category, button);
    ...
    _load() {
        this._addCategoryButton(this._appsBackend.frequentAppsCategory());
        this._addCategoryButton(this._appsBackend.recentFilesCategory());
        this._separator = new St.Widget({
            style_class: 'hybrid-categories-separator',
            x_expand: true,
        });
        this._categoriesBox.add_child(this._separator);
        super._load();
        this._setActiveButton(this._categoriesBox.get_first_child());
    }

    _selected(categoryItem, categoryMenuId) {
        this._setActiveButton(categoryItem);
        super._selected(categoryItem, categoryMenuId);
    }

    _setActiveButton(button) {
        this._activeButton?.remove_style_pseudo_class('active');
        this._activeButton = button;
        this._activeButton?.add_style_pseudo_class('active');
    }
...
const CategoryAppsMenu = class extends PopupMenu.PopupMenu {
    constructor(
        ...
        inheritLightStyle,
        onActivated) {
        const side = openOnRight
            ? St.Side.LEFT
            : (sourceActor.get_text_direction() === Clutter.TextDirection.RTL
                ? St.Side.LEFT
                : St.Side.RIGHT);
        ...
        this._inheritLightStyle = inheritLightStyle;
    ...
    _syncLightStyle() {
        ...
        while (actor) {
            if (actor.has_style_class_name?.('community-menu-light')) {
                lightStyle = true;
                break;
            }
        ...
        if (lightStyle)
            this.actor.add_style_class_name('community-menu-light');
        else
            this.actor.remove_style_class_name('community-menu-light');
    }
...
export const AppsListSection = GObject.registerClass({
    ...
    _init(
        ...
        compactSearch = false,
        gridColumns = Constants.COLUMN_COUNT) {
        ...
        if (isGrid) {
            this.grid = new Widgets.Grid(gridColumns, Constants.COLUMN_SPACING, Constants.ROW_SPACING);

...
export const SearchEntry = GObject.registerClass({
    ...
    _init(searchResults) {
        ...
        this._text.connectObject('activate', this._onActivate.bind(this), this);
    ...
    startSearch(event) {
        ...
        if (typeof this._text?.set_input_interceptor === 'function') {
            const symbol = event.get_key_symbol();
            const unicode = Clutter.keysym_to_unicode(symbol);

            if (unicode !== 0) {
                this._text.insert_unichar(String.fromCodePoint(unicode));
                return;
            }
        }
    ...
    _onMapped() {
        ...
        this._text.set_input_interceptor(this.mapped ? global.stage : null);
    }

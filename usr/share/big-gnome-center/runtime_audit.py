...
"""Read-only audit for the accepted Big Gnome Center Shell baseline."""
...
def _payload_hashes(root: Path) -> dict[str, str]:
    ...
    hashes["runtime-audit-tool"] = _tree_hash(
        root / "usr/share/big-gnome-center/runtime_audit.py"
    )
    hashes["runtime-baseline-tool"] = _tree_hash(
        root / "usr/share/big-gnome-center/runtime_baseline.py"
    )
    return hashes
...
def _print_human(snapshot: Snapshot, checks: list[Check]) -> None:
    print("Big Gnome Center runtime audit")
